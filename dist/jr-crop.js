...
(function (window, angular, ionic, undefined) {
    ...
    function jrCropFactory ($ionicModal, $rootScope, $q) {
         ...
         */
        function resizeCanvas (canvas, W, H, W2, H2) {
            var time1        = Date.now();
            W2               = Math.round(W2);
            H2               = Math.round(H2);
            var img          = canvas.getContext("2d")
                                     .getImageData(0, 0, W, H);
            var img2         = canvas.getContext("2d")
                                     .getImageData(0, 0, W2, H2);
            var data         = img.data;
            var data2        = img2.data;
            var ratio_w      = W / W2;
            var ratio_h      = H / H2;
            ...
            canvas.getContext("2d")
                  .clearRect(0, 0, Math.max(W, W2), Math.max(H, H2));
            canvas.width  = W2;
            canvas.height = H2;
            canvas.getContext("2d")
                  .putImageData(img2, 0, 0);
        }
        ...
        function resize (canvas, W, H, W2, H2) {
            return resizeCanvas (canvas, W, H, W2, H2);
        }
    ...
    jrCropFactory.$inject = ["$ionicModal", "$rootScope", "$q"];
